				...
				<div class="card" style="border-right: 5px solid {!! $order->status_color !!}">
					<div class="card-body" style="background:#fff;">
						...
						@switch($viewSettings->display['card_line_3'])
							...
							@case(2)<h6 class="card-subtitle text-muted mt-1">{{ ($order->address) }}@if($order->type == 'delivery')<a class="p-2 fab fa-google" target="_blank" title="Google Maps" href="{{ 'https://www.google.com/maps/dir/?api=1&destination='.urlencode($order->address) }}"></a><a class="p-2 fab fa-bing" target="_blank" title="Bing Maps" href="{{ 'https://maps.bing.com/?api=1&destination='.urlencode($order->address) }}"><img src="https://www.bing.com/sa/simg/favicon-2x.ico" width="15" height="15" ></a>@endif</h6>@break
							...
							@case(4)<h6 class="card-subtitle text-muted mt-1">{{ ($order->address) }}@if($order->type == 'delivery')<a class="p-2 fab fa-bing" target="_blank" title="Bing Maps" href="{{ 'https://maps.bing.com/?api=1&destination='.urlencode($order->address) }}"><img src="https://www.bing.com/sa/simg/favicon-2x.ico" width="15" height="15" ></a>@endif</h6>@break
						@endswitch
