			        ...
			        if ($total->code == 'total' || $total->code == 'order_total')
					{
						...
						$this->vars['results'][] = (object)[
							...
							'buttons' => '
								...
								'.($viewSettings->display['button4_enable'] ? '<a class="btn label-default'.($order->status_id != $viewSettings->display['button4_status'] ? '" href="'.$buttonUrl.$viewSettings->display['button4_status'].'" style="background-color:'.$viewSettings->display['button4_color'].'";' : ' btn-light"').'>'.$viewSettings->display['button4_text'].'</a>' : '').'
								'.($viewSettings->display['button5_enable'] ? '<a class="btn label-default'.($order->status_id != $viewSettings->display['button5_status'] ? '" href="'.$buttonUrl.$viewSettings->display['button5_status'].'" style="background-color:'.$viewSettings->display['button5_color'].'";' : ' btn-light"').'>'.$viewSettings->display['button5_text'].'</a>' : '').'
							',
